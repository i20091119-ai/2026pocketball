...
(function (global) {
  ...
  mode.init = function () {
    ...
    mode.el.angles.addEventListener('click', () => { mode.showAngles = !mode.showAngles; mode.el.angles.classList.toggle('on', mode.showAngles); mode.el.angles.textContent = mode.showAngles ? '📐 각도 끄기' : '📐 각도 보기'; if (mode.showAngles) global.App.msg('📐 닿은 점의 접선이 거울이에요. 법선이 두 각을 반으로 나누고, 두 초점을 잇는 각도 반으로 나눠요'); });
  ...
  mode.render = function (ctx, t) {
    ...
    if (mode.showAngles) { const m = mode.marks[0]; if (m && m.din) R.drawAngles(ctx, m.cx, m.cy, m.nx, m.ny, m.din, m.out, { tangent: true, foci: bd.foci(), rim: { x: m.x, y: m.y }, ballR: BR }); } // 첫 번째 쿠션만
    // 조준선
    if (mode.aim && !world.anyMoving()) {
      ...
      if (a && mode.showAngles && !mode.marks.length) {
        ...
        if (hit && hit.kind === 'wall') { const vn = a.dx * hit.nx + a.dy * hit.ny; R.drawAngles(ctx, hit.x, hit.y, hit.nx, hit.ny, { x: a.dx, y: a.dy }, { x: a.dx - 2 * vn * hit.nx, y: a.dy - 2 * vn * hit.ny }, { tangent: true, foci: bd.foci(), rim: { x: hit.x - hit.nx * BR, y: hit.y - hit.ny * BR }, ballR: BR }); }
      }
      if (a) {
